---
title: thaw
description: Git for live LLM agent sessions. Checkpoint, branch, diff, and restore live vLLM/SGLang inference state, so a session forks in 0.88s median instead of a ~340s cold boot. Built in Rust, CUDA, and Python.
year: 2026
...
thaw treats a running agent session the way git treats a working tree. It turns the live inference state of a vLLM or SGLang session, the weights, the KV cache, the scheduler, and the prefix-hash table, into a durable file you can `checkpoint`, `branch`, `diff`, `checkout`, and `log`. A new worker forks from a checkpoint in 0.88s median rather than booting from nothing, and the inspection verbs, `inspect`, `diff`, and `log`, need no GPU at all, so the everyday loop runs on a laptop. It is open source, on PyPI as `thaw-vllm` (sixteen releases and counting), and built in Rust, CUDA, and Python.
...
**Testing GPU code without a GPU.** All of this is CUDA, and I write most of it on a Mac with no CUDA toolchain. The `CudaBackend` trait puts a mock backend and the real CUDA backend behind one contract, so the logic, the pipeline ordering, the shard math, the snapshot bookkeeping, runs and gets tested without a GPU in the loop. 388 tests run in CI, 155 Rust and 233 Python, on macOS and Linux with no CUDA installed. The real backend has to satisfy the same contract the mock does, so the tests are about the thing, not a stand-in for it.

**Rewind.** The newest layer is `thaw rewind`, the rollout-level companion to the session verbs. Capture N sampled continuations from a shared trunk with per-token logprobs, then find the exact token where they diverged and which branch the model was most confident in, all on a laptop with no GPU. It is the primitive under best-of-N and RL credit-assignment work, validated end to end on an A100 with Qwen2.5-7B best-of-8. The same question, what does re-feeding a transcript actually cost versus resuming exact KV state, became a sole-author research preprint, *Re-feeding Is Not Replaying: Measuring Replay Noise in Counterfactual Token-Credit Estimation* (June 2026).
...
| 388 tests in CI | 155 Rust + 233 Python, GPU logic without a GPU | `CudaBackend` trait, mock and real behind one contract | [repo](https://github.com/thaw-ai/thaw) |
...
thaw is open source under Apache-2.0 and on PyPI as [`thaw-vllm`](https://pypi.org/project/thaw-vllm/), sixteen releases in, currently 0.6.0. The cold-start problem it composes with is also being worked in the open: I'm an active participant in vLLM [RFC #34303](https://github.com/vllm-project/vllm/issues/34303), *CUDA Checkpoint/Restore for Near-Zero Cold Starts*, where the RFC author asked five of us in the thread for input on direction; out of it I opened [PR #44074](https://github.com/vllm-project/vllm/pull/44074), a pluggable sleep-mode backend abstraction, with receipted 70B sleep/wake integration on 2×H100. The research side became a sole-author preprint, *Re-feeding Is Not Replaying* (June 2026). I am the founder and lead engineer, 2026 to present, full-time on it under Matteson Systems LLC. I applied to YC for the S26 batch and got rejected; the application placed in the top 10% and YC encouraged a reapply, so I am reapplying.
