...
My main project is [thaw](/work/thaw), git for live LLM agent sessions, built in Rust, CUDA, and Python: it checkpoints, branches, diffs, and restores live inference state (weights, KV cache, prefix-hash table, scheduler), and forks a running vLLM session in 0.88s median on an H100, against a cold boot of roughly 340s. Getting there meant a double-buffered O_DIRECT DMA pipeline that overlaps disk reads with PCIe transfer, an 8-shard parallel CRC32C verifier that matches a serial pass exactly, and coalescing about 16,000 tiny per-block DMAs into one contiguous gather to kill a 60x snapshot bottleneck. It is sixteen releases deep on PyPI as `thaw-vllm`, with an upstream vLLM PR and a sole-author research preprint behind it, and 388 tests in CI (155 Rust, 233 Python) that run with no CUDA toolchain installed.
